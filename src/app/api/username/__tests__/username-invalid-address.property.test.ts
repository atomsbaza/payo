...
/**
 * Feature: public-profile-page, Property 5: Invalid Ethereum address rejection
 *
 * For any string that does not match the Ethereum address format
 * (/^0x[a-fA-F0-9]{40}$/), calling PUT /api/username/[address]
...
/** 40 hex chars — valid body used to build near-miss cases */
const hex40Arb = fc
...
const nonHexCharArb = fc.constantFrom(
  ...'ghijklmnopqrstuvwxyz!@#$%^&*()_+-=[]{}|;:,.<>?/~`'.split(''),
)
...
/** Completely random strings (filtered to exclude valid addresses) */
...
// --- Simulate the Username API route's address validation ---
...
describe('Feature: public-profile-page, Property 5: Invalid Ethereum address rejection', () => {
  /**
   * Any string that does not match /^0x[a-fA-F0-9]{40}$/ must be rejected
   * with HTTP 400 and the expected error message by the Username API.
   * **Validates: Requirements 2.3**
   */
